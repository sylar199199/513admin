<template>
  <c-nest-layout parent-router-name="announcements">
...
export default {
  ...
  methods: {
    ...mapActions('announcements', {
      handleGetDataByCondition: 'getAllBulletin',
    }),
    selectChange(val) {
  ...
  mounted() {
    this.value = '3'
    this.status = this.$qs.parse(this.$route.query.search).status || ''
    this.$router.push({
      query: {
        search: this.$qs.stringify({
          msgType: this.value === '' ? '0' : this.value,
