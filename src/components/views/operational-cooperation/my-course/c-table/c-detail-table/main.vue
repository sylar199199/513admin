...
import tableConfig from '@components/views/operational-cooperation/my-course/c-table/c-detail-table/table-config'
...
export default {
  ...
  computed: {
    ...mapState('my-course', {
      // 表格数据
      tableData: state => {
        if (state.cooperateDetail[0] === undefined) {
          return []
        } else {
      ...
      this.$msgbox({
        ...
        beforeClose: async (action, instance, done) => {
          if (action === 'confirm') {
            ...
            if (res.ok) {
              let params = {}
              params.courseIdSign = this.schoolIdSignState //课程签名
              params.providerIdSign = this.courseIdSignState //提供学校签名
